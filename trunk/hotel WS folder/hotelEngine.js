...
hfrm.action= "/folder/dump.html" //"requestTest.php"
...
htlForm.datagridMatrix = document.createElement("DIV")
//Able.style.display = "block"
document.body.appendChild(Able)
//htlForm.datagridMatrix.appendChild(Able)
...
//										  alert("selected Hotel")
										  hotelSelected("test",q)
										}
...
//alert("doBook : " + htlForm.hotelInfo.recordCount())
//var doc = this.data
doc.reset()

	var hotelTrackId = doc.getFld('HOTELID') //getNodeValue(xmlDoc,"hotelTrackId")
//	alert(11)
	var searchId = doc.getFld('searchId') //htlForm.searchId.toString()
//alert(searchId)
//	alert(21)
...
//alert("Pre Rd : " + doc.getFld('COMBINATIONID'))
...
{
//alert("BC" + htlForm.hotelInfo.recordCount())
htlForm.hotelInfo.pointer = 0
...
paxDetailsForm.action= "/folder/bookingsucess.xml" //"requestTest.php"
...
for (var i=0;i<rc;i++)
{
		...
		elm.value = doc.getFld("ROOMCODE") //getNodeValue(rooming[i],"roomcode")
		dv.appendChild(elm)
...
{
	doc.nextRecord()
alert((doc.getFld('FLEXIHOTELNAME')))
...
//alert("Next Record : " + this[returnArray.pointer + 1] + "\nPointer + "+returnArray.pointer)
		if (returnArray.EOF() == false)
		{
...
returnArray.reset = function(){
//alert("Resetting Done")
returnArray.pointer = 0
}
...
//	alert(colName + "\n Total : " + searchAble.rows[0].cells.length + "\n Col Index : " + colIndx)
//alert(this[this.pointer])
//alert(searchAble.innerHTML)

return searchAble.rows[this[this.pointer]].cells[colIndx].innerHTML
}
